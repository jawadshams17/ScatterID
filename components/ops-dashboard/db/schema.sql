-- ScatterID Ops & Access Database Schema (SQLite WAL Mode)
...
-- 2. Users Table (Help Desk Clerks, Moderators, and Root Admins)
...
-- 3. Recovery Codes Table (8 single-use codes per administrator)
...
-- 4. Requests Moderation & Intake Queue (Scenario B: Tiered Risk Routing)
...
-- 5. Audit Log Table (Immutable audit attribution stamp across all actors)
...
-- 6. Gateway Keys Table (Zero-downtime dual-key phasing for API keys)
...
-- 7. PQC Key Pool Table (Advance Pre-Distribution ML-DSA-87 / ML-DSA-65 keys)
...
-- 8. Cryptographic Delegation Endorsements Table (Emergency Fallback Tokens)
